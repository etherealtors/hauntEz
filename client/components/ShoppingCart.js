...
class ShoppingCart extends Component {
  ...
  async applyDiscount(event) {
    ...
    await this.props.applyDiscount(
      this.props.cart[0].orderId,
          ...
          {cart
            ? cart.map(item => {
                return item.location ? (
                  <div key={item.id}>
                    <h2>
                      {item.location.address} for ${item.location.price} x{' '}
                      {item.quantity} = ${item.price * item.quantity}
                      {item.quantity > item.location.quantity ? (
                        <p>
                          *Low on stock. Please reduce the number of items and
                          try again
                        </p>
                      ) : null}
                    </h2>
